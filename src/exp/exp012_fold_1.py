...
LLM_TEXT_PATH = HOME_PATH / Path("results/exp105_train_add_text.csv")
    ...
    for n in FOLDS:
        ...
        for epoch in range(n_epochs):
            ...
            for i, d in tqdm(enumerate(train_loader), total=len(train_loader)):
                d1 = d["inputs1"]
                misconception_ = d["misconception"]
                d1 = collate_sentence(d1)
                negative_miconception_ = [
                    i for i in negative_misconception if i not in misconception_.numpy()]
                np.random.shuffle(negative_miconception_)
                negative_miconception_ = negative_miconception_[:negative_size]
                train_misconception_ = np.concatenate(
                    [misconception_.numpy(), negative_miconception_])
                train_misconception_ = np.unique(train_misconception_)
                target = []
                for i in misconception_.numpy():
                    label_index = np.where(train_misconception_ == i)[0][0]
                    target.append(label_index)
                passage_input_ids = []
                passage_attention_masks = []
                for p in train_misconception_:
                    inputs = tokenizer(
                        misconception_name[p],
                        max_length=256,
                        padding="max_length",
                        truncation=True,
                        return_attention_mask=True,
                    ...
                    passage_input_ids.append(inputs["input_ids"])
                    passage_attention_masks.append(inputs["attention_mask"])
                passage_input_ids = torch.tensor(
                    passage_input_ids, dtype=torch.long)
                passage_attention_masks = torch.tensor(
                    passage_attention_masks, dtype=torch.long)
                d2 = {'input_ids': passage_input_ids,
                      "attention_mask": passage_attention_masks}
                ...
                with autocast():
                    q_reps = model(ids1, mask1)
                    p_reps = model(ids2, mask2)
                    q_reps = torch.nn.functional.normalize(q_reps, dim=-1)
                    p_reps = torch.nn.functional.normalize(p_reps, dim=-1)
                    scores = compute_similarity(q_reps, p_reps) * 20
                    # print(scores.shape)
                    scores = scores.view(q_reps.size(0), -1)
                    target = torch.tensor(
                        target, device=scores.device, dtype=torch.long)
                    loss = compute_loss(scores, target)
